...
def create_and_send_outreach_draft(lead_name: str, contact_name: str = None, chat_id: str = None):
    ...
    """
    try:
        frappe.db.savepoint("docvision_create_outreach")
        ...
        condition = getattr(setting, "outreach_draft_condition", None)
        if recipient_email and condition:
            lead.reload()
            if contact:
                contact.reload()
            if not evaluate_outreach_condition(condition, lead, contact):
                if chat_id:
                    send_telegram_message(
                        chat_id,
                        "ℹ️ Outreach condition was not met, so the email draft was skipped.",
                    )
                return None
        ...
        frappe.db.rollback(save_point="docvision_create_outreach")
...
def approve_and_send(outreach_name: str, chat_id: str, message_id: str, from_user: dict):
    ...
    try:
        ...
        _assert_matching_chat(outreach, chat_id)
        
        if outreach.status in {"Sending", "Sent"}:
        ...
        updated_text += f"\n\n✅ <b>Approved & Sent</b> by {escape_telegram(user_name)} on {escape_telegram(timestamp)}"
        ...
        return "Failed to send email. Check the Error Log.", True
...
def prompt_revision(outreach_name: str, chat_id: str, message_id: str):
    ...
    try:
        frappe.db.savepoint("docvision_prompt_revision")
        if not frappe.db.exists("DocVision Outreach", outreach_name):
            return "Outreach record not found.", True
        ...
        prompt_text = (
            f"✏️ <b>Revision Request</b> for <b>{escape_telegram(outreach.company_name or outreach.lead)}</b>\n\n"
        ...
        frappe.db.rollback(save_point="docvision_prompt_revision")
...
def process_revision_reply(outreach_name: str, chat_id: str, revision_text: str, from_user: dict):
    ...
    try:
        frappe.db.savepoint("docvision_revision_reply")
        if not frappe.db.exists("DocVision Outreach", outreach_name):
            return None
        ...
        frappe.db.rollback(save_point="docvision_revision_reply")
...
def _lock_lead(lead_name: str) -> None:
    ...
    if not rows:
        frappe.throw("Lead was not found.")


def _assert_matching_chat(outreach, chat_id: str | int | None) -> None:
    expected_chat_id = str(outreach.telegram_chat_id or "")
    actual_chat_id = str(chat_id or "")
    if not expected_chat_id or not hmac.compare_digest(expected_chat_id, actual_chat_id):
        raise frappe.PermissionError(
            "Outreach action does not belong to this Telegram chat"
        )
...
def _html_to_telegram(value: str) -> str:
    """Convert untrusted email HTML to escaped Telegram text."""
    return html_to_safe_telegram_text(value)
...
def _format_outreach_preview_message(outreach, is_revised=False):
    ...
    msg = f"{header}\n\n"
    msg += f"👤 <b>To:</b> {escape_telegram(outreach.recipient_email)}\n"
    if outreach.company_name:
        msg += f"🏢 <b>Company:</b> {escape_telegram(outreach.company_name)}\n"
    if outreach.lead_source:
        msg += f"🏷️ <b>Source:</b> {escape_telegram(outreach.lead_source)}\n"
    msg += f"📌 <b>Lead:</b> {telegram_form_link('Lead', outreach.lead, outreach.lead)}\n"
    msg += f"\n📝 <b>Subject:</b> {escape_telegram(outreach.subject)}\n\n"
...
def _format_research_only_message(outreach) -> str:
    ...
    if outreach.company_name:
        msg += f"🏢 <b>Company:</b> {escape_telegram(outreach.company_name)}\n"
    if outreach.website:
        msg += f"🌐 <b>Website:</b> {escape_telegram(outreach.website)}\n"
    if outreach.industry:
        msg += f"🏭 <b>Industry:</b> {escape_telegram(outreach.industry)}\n"
    msg += f"📌 <b>Lead:</b> {telegram_form_link('Lead', outreach.lead, outreach.lead)}\n"

    if outreach.company_research:
        overview = escape_telegram(outreach.company_research)
        if len(overview) > 800:
            overview = overview[:800] + "... [truncated]"
        msg += f"\n🏢 <b>Company Research:</b>\n{overview}\n"

    if outreach.person_research:
        person = escape_telegram(outreach.person_research)
        if len(person) > 600:
